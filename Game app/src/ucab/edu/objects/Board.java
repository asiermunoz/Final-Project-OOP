...
public class Board implements Show{
    ...
    public void show() {
        ...
        for (int i = 0; i < length; i++){
            ...
            for (int j = 0; j < length; j++){
                String place;
                if(table[i][j].letter.getLetter().length() == 1){
                    if(table[i][j].letter.isJoker()){
                        place = table[i][j].color + "☻ " + ANSI_RESET;
                    }
                    else {
                        place = table[i][j].color + table[i][j].letter.getLetter() + " " + ANSI_RESET;
                    }
                }
                else{
                    place = table[i][j].color + table[i][j].letter.getLetter() + ANSI_RESET;
